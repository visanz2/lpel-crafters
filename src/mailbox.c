...
struct mailbox_t {
	ocr_pthread_mutex_t  lock_free;
	ocr_pthread_mutex_t  lock_inbox;
	ocr_pthread_cond_t   notempty;
...
static mailbox_node_t *GetFree( mailbox_t *mbox)
{
  ...
  ocr_pthread_mutex_lock( &mbox->lock_free);
  ...
  ocr_pthread_mutex_unlock( &mbox->lock_free);
...
mailbox_t *LpelMailboxCreate(void)
{
  ...
  ocr_pthread_mutex_init( &mbox->lock_free,  NULL);
  ocr_pthread_mutex_init( &mbox->lock_inbox, NULL);
  ocr_pthread_cond_init(  &mbox->notempty,   NULL);
  mbox->list_free  = NULL;
...
void LpelMailboxDestroy( mailbox_t *mbox)
{
  ...
  #if 0
  ocr_pthread_mutex_lock( &mbox->lock_inbox);
  ...
  ocr_pthread_mutex_unlock( &mbox->lock_inbox);
  #endif
  ...
  ocr_pthread_mutex_destroy( &mbox->lock_free);
  ocr_pthread_mutex_destroy( &mbox->lock_inbox);
  ocr_pthread_cond_destroy(  &mbox->notempty);
...
void LpelMailboxSend( mailbox_t *mbox, workermsg_t *msg)
{
  ...
  if ( mbox->list_inbox == NULL) {
    ...
    ocr_pthread_cond_signal( &mbox->notempty);
...
void LpelMailboxRecv( mailbox_t *mbox, workermsg_t *msg)
{
  ...
  ocr_pthread_mutex_lock( &mbox->lock_inbox);
  while( mbox->list_inbox == NULL) {
	  ocr_pthread_cond_wait( &mbox->notempty, &mbox->lock_inbox);
  }

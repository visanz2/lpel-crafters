...
#else /* HAVE___THREAD */
static ocr_pthread_key_t workerctx_key;
#endif /* HAVE___THREAD */
/******************************************************************************/
...
void initLocalVar(int size){
	...
	ocr_pthread_key_create(&workerctx_key, NULL);
#endif /* HAVE___THREAD */
...
#ifndef HAVE___THREAD
	ocr_pthread_key_delete(workerctx_key);
#endif /* HAVE___THREAD */
...
#else /* HAVE___THREAD */
	/* set pointer to worker context as TSD */
	ocr_pthread_setspecific(workerctx_key, wp);
#endif /* HAVE___THREAD */
...
workerctx_t *LpelCreateWrapperContext(int wid) {
	...
	(void) ocr_pthread_create(&wp->thread, NULL, WrapperThread, wp);
	(void) ocr_pthread_detach(wp->thread);
	return wp;
}
...
#else /* HAVE___THREAD */
  /* set pointer to worker context as TSD */
  ocr_pthread_setspecific(workerctx_key, wc);
#endif /* HAVE___THREAD */
...
#else /* HAVE___THREAD */
  return (workerctx_t *) ocr_pthread_getspecific(workerctx_key);
#endif /* HAVE___THREAD */
}

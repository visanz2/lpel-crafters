...
void *LpelStreamRead( lpel_stream_desc_t *sd)
{
  ...
  /* quasi P(n_sem) */
  int atomic_value = atomic_load(&sd->stream->n_sem);
  //printf("READatomic_value %d on %d\n", atomic_value, sd->stream->uid);
  if ( atomic_fetch_sub( &sd->stream->n_sem, 1) == 0) {
	  atomic_value = atomic_load(&sd->stream->n_sem);
	  //printf("READatomic_value2 %d on %d\n", atomic_value, sd->stream->uid);
#ifdef USE_TASK_EVENT_LOGGING
    /* MONITORING CALLBACK */
...
void LpelStreamWrite( lpel_stream_desc_t *sd, void *item)
{
  ...
  /* quasi V(n_sem) */
  int atomic_value = atomic_load(&sd->stream->n_sem);
  //printf("WRITEatomic_value %d on %d\n", atomic_value, sd->stream->uid);
  if ( atomic_fetch_add( &sd->stream->n_sem, 1) < 0) {
	  atomic_value = atomic_load(&sd->stream->n_sem);
	  	  //printf("WRITEatomic_value2 %d on %d\n", atomic_value, sd->stream->uid);
    /* n_sem was -1 */

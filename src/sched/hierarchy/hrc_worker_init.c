...
void LpelWorkersCleanup(void) {
	...
	for(i=0; i<num_workers; i++) {
		...
		(void) ocr_pthread_join(wc->thread, NULL);
	}
	/* wait for the master to finish */
	(void) ocr_pthread_join(master->thread, NULL);
...
void LpelWorkersSpawn(void) {
	int i;
	/* master */
	(void) ocr_pthread_create(&master->thread, NULL, MasterThread, master); 	/* spawn joinable thread */

	/* workers */
	for(i=0; i<num_workers; i++) {
		workerctx_t *wc = workers[i];
		(void) ocr_pthread_create(&wc->thread, NULL, WorkerThread, wc);
	}
}
